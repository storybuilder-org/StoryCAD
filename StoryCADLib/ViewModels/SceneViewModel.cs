...
public class SceneViewModel : ObservableRecipient, INavigable, ISaveable
{
    ...
    public SceneViewModel(ILogService logger, AppState appState)
    {
        ...
        try
        {
            ...
            foreach (var purpose in ScenePurposeList)
            {
                ScenePurposes.Add(new StringSelection(purpose));
            }
    ...
    private void InitializeCharacterList()
    {
        ...
        foreach (var _element in CastSource)
        {
    ...
    public void SaveModel()
    {
        ...
        foreach (var element in CastList)
        {
            Model.CastMembers.Add(element.Uuid);
        }

        Model.ScenePurpose.Clear();
        foreach (var purpose in ScenePurposes)
        {
    ...
    {
        OnPropertyChanged();
        var msg = $"Add Scene Purpose {selectedPurpose.StringName}";
    ...
    {
        OnPropertyChanged();
        var msg = $"Remove Scene Purpose {selectedPurpose.StringName}";
        Messenger.Send(new StatusChangedMessage(new StatusMessage(msg, LogLevel.Info, true)));
    }
    ...
    public void SwitchCastView(object sender, RoutedEventArgs e)
    {
        ...
        {
            Messenger.Send(
                new StatusChangedMessage(new StatusMessage("Add / Remove Cast Members", LogLevel.Info, true)));
        }
        else
        {
            Messenger.Send(
                new StatusChangedMessage(new StatusMessage("Show Selected Cast Members", LogLevel.Info, true)));
        }
    ...
    public void AddCastMember(StoryElement element)
    {
        ...
        OnPropertyChanged();
        Messenger.Send(
            new StatusChangedMessage(new StatusMessage($"New cast member {element.Name} added", LogLevel.Info, true)));
    }

    public void RemoveCastMember(StoryElement element)
    {
        for (var _i = 0; _i <= CastList.Count - 1; _i++)
        {
            if (CastList[_i].Uuid == element.Uuid)
            {
                ...
                OnPropertyChanged();
                Messenger.Send(
                    new StatusChangedMessage(new StatusMessage($"Cast member {element.Name} removed", LogLevel.Info,
                        true)));
                return;
            }
        }
    ...
    private void UpdateViewpointCharacterTip()
    {
        ...
        if (!shellModel.StoryElements.StoryElementGuids.TryGetValue(node.Uuid, out var element) ||
            element is not OverviewModel overview)
        {
            _logger.Log(LogLevel.Warn, $"OverviewModel not found for node with UUID: {node.Uuid}");
            return;
        }

        var viewpointTip = string.IsNullOrEmpty(overview.Viewpoint)
            ? "No story viewpoint selected"
            : $"Story viewpoint = {overview.Viewpoint}";

        var viewpointCharacterTip = "No story viewpoint character selected";
