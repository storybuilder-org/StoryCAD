...
namespace StoryCAD.Services.Reports
{
	...
	public class RichTextStripper
	{
		...
		// Updated regex to include newline characters in the tchar group
		...
		private static readonly Dictionary<string, string> specialCharacters = new()
		{
			{ "par", "\n" },
			{ "line", "\n" },
			{ "pard", "\n" }, // Added to handle \pard as newline
            { "sect", "\n" },
			{ "page", "\n" },
			{ "tab", "\t" },
			{ "emdash", "\u2014" },
		...
		public string StripRichTextFormat(string inputRtf)
		{
			...
			if (matches.Count > 0)
			{
				foreach (Match match in matches)
				{
					...
					string brace = match.Groups[4].Value;
					string tchar = match.Groups[5].Value;
			...
			returnString = string.Concat(outList);
