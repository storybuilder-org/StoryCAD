...
public class ShellViewModel : ObservableRecipient
{
    ...
    public async Task OpenFile(string fromPath = "")
    {
        ...
        try
        {
            ...
            if (fromPath == "" || !File.Exists(fromPath))
            {
                ...
                InitializeWithWindow.Initialize(filePicker, GlobalData.WindowHandle);
    ...
    private async void Preferences()
    {
        ...
        else { Messenger.Send(new StatusChangedMessage(new("Preferences closed", LogLevel.Info, true))); }
    }
    ...
    private async void MasterPlotTool()
    {
        ...
        if (result == ContentDialogResult.Primary)   // Copy command
        {
            ...
            {
                SceneModel child = new SceneModel(StoryModel) { Name = scene.SceneTitle, Remarks = "See Notes.", Notes = scene.Notes };
    ...
    private async void DramaticSituationsTool()
    {
        ...
        if (RightTappedNode == null)
        {
            ...
            Messenger.Send(new StatusChangedMessage(new StatusMessage(msg, LogLevel.Warn)));
            return;
        }
        switch (result)
        {
    ...
    /// <summary>
    /// Launches user manual in the default browser
    /// </summary>
    private void LaunchGitHubPages()
    {
        ...
        //Starts the users browser to the manual
        Process.Start(new ProcessStartInfo { FileName = @"https://storybuilder-org.github.io/StoryBuilder-2/", UseShellExecute = true });
    ...
    private void MoveTreeViewItemLeft()
    {
        ...
        {
            Messenger.Send(new StatusChangedMessage(new("That move isn't valid!", LogLevel.Info)));
            return;
        }
    ...
    private async void RemoveStoryElement()
    {
        ...
        {
            if (Ioc.Default.GetRequiredService<DeletionService>().SearchStoryElement(node, RightTappedNode.Uuid, StoryModel))
            {
                FoundNodes.Add(node);
            }
        }
        ...
        if (delete)
        {
            ...
            if (CurrentView.Contains("Explorer")){ RightTappedNode.Delete(Models.ViewType.Explorer); }
