...
Scene Builder does not create a Scene. Make the scene first, in StoryCAD or as a stub from Problem Builder, then pick it here. You can also run it on the story problem, the Spine of the outline. When you pick a scene, the run reads the Problem whose beat that scene sits on, so a scene assigned to a beat gets better suggestions than one floating free, and it proposes what it can infer. On the Scene tab: the Scene Sketch, the Viewpoint character, the Setting, the Cast, and the Date and Time. On the Development tab: the Purpose of Scene, the Scene Type, and the questions that follow them, what value changes hands, what happens, what follows from it, why it matters, and what the character realizes. On the Conflict tab: the Protagonist and Antagonist, their Goals, the Opposition, the Outcome, and what each of them feels. On the Sequel tab: the Emotional Response, the Review and Thought, and the New Goal. Anything else goes in Notes.

The Cast, the Viewpoint character, and the two roles on the Conflict tab are chosen from the characters in your outline; Scene Builder does not invent people. Like Character Builder, this is a long list of proposals, and Review Each is the comfortable way through it. It is one of the five starred by default.

![Property Updates after Scene Builder on The missing seat, a stub Problem Builder created](../../media/Collaborator/Collaborator-Workflow-Scene-Builder.png)
