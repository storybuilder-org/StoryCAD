...
| 9 | `StoryCADLib/Collaborator/Views`: `WorkflowShell` (5), `WorkflowPage` (10), `ElementPicker` (5), `CollaboratorHostRoot` (0) | 20 | WorkflowPage's data-bound workflow content takes templated-Name treatment; convert `AnnotatedFiles` to all-scope-files, making the coverage test the permanent fitness function. ElementPicker is a near-clone of `Services/Dialogs/ElementPicker.xaml` sharing its literal `x:Name` values (`TypeBox`, `ElementBox`, `NewButton`) — its ids need a distinct prefix (for example `CollabPicker...`) or the Uniqueness test collides with Unit 6's ids (flagged on PR #1455). WorkflowShell's NavigationView counts under the Nav/NavItem suffix rows added in Unit 6: container id only, its items are `MenuItemsSource`-bound |
