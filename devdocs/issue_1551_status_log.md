...
| Fix | A one-way `SelectedItem` binding to the same property on every editable ComboBox: Problem 12, Character 23, Scene 9, Setting 2, and the Trait and Attitude boxes in `RelationshipView` (2). XAML only, no C# change. Overview and StoryWorld have no editable boxes. |
...
| Tests | `StoryCADTests/Xaml/EditableComboBoxSelectionTests.cs`: static XAML scan, every editable ComboBox with an x:Bind on Text carries a one-way SelectedItem binding to the same property. Passes on the branch; reports 48 violations with the fix stashed. The 17 blank-row tests were reverted with that code. |
